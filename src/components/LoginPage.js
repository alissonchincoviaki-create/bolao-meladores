  ...
  const handleFirstAccess = async () => {
    ...
    try {
      ...
      await supabase.from('users').update({
        password_hash: newPassword, first_access: false,
        avatar_url_1: avatarUrl || photoPreview, avatar_choice: 1,
      }).eq('id', userData.id);
      ...
      await supabase.from('users').update({
        password_hash: newPassword, first_access: false,
        avatar_url_1: photoPreview, avatar_choice: 1,
      }).eq('id', userData.id);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-dark-50 to-primary-50 p-5">
        <div className="bg-white rounded-2xl p-8 max-w-sm w-full shadow-xl border border-primary-200 flex flex-col items-center">
          ...
          <div className="w-full mb-4"><label className="block text-sm font-semibold text-dark-700 mb-1">Nova senha</label><input type="password" placeholder="Mínimo 6 caracteres" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="input-field" /></div>
          <div className="w-full mb-4"><label className="block text-sm font-semibold text-dark-700 mb-1">Confirmar senha</label><input type="password" placeholder="Repita a senha" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="input-field" /></div>
          <div className="w-full mb-4">
            <label className="block text-sm font-semibold text-dark-700 mb-2">📸 Sua foto (obrigatório)</label>
            <div className="flex items-center gap-4">
              {photoPreview ? (
                <div className="w-20 h-20 rounded-full overflow-hidden border-3 border-primary-600 flex-shrink-0"><img src={photoPreview} alt="Preview" className="w-full h-full object-cover" /></div>
              ) : (
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-dark-50 to-primary-50 p-5">
      <div className="bg-white rounded-2xl p-8 max-w-sm w-full shadow-xl border border-primary-200 flex flex-col items-center">
        ...
        <form onSubmit={handleLogin} className="w-full">
          <div className="w-full mb-4"><label className="block text-sm font-semibold text-dark-700 mb-1">Apelido</label><input type="text" placeholder="Seu apelido" value={login} onChange={e => setLogin(e.target.value)} className="input-field" required /></div>
          <div className="w-full mb-4"><label className="block text-sm font-semibold text-dark-700 mb-1">Senha</label><input type="password" placeholder="Sua senha" value={password} onChange={e => setPassword(e.target.value)} className="input-field" required /></div>
          {error && <p className="text-danger text-sm mb-3 text-center">{error}</p>}
