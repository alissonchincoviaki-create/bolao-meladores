...
import { getFlag, GROUPS, DEADLINES, getTimeRemaining, isDeadlinePassed, matchHasStarted, getMatchDateTime } from '@/lib/scoring';
import ScoreStepper from './ScoreStepper';
import FlagSelect from './FlagSelect';
import Avatar from './Avatar';

const SCARY_PHRASES = [
  "Tô de olho nos seus palpites... 👁️", "Tenta me alcançar... se conseguir 😈",
  "Eu assombro esse ranking 👻", "Palpita com medo, palpita... 🫣",
  "O líder nunca dorme 💀", "Sente a pressão? Sou eu 😏",
  "Seu palpite tá tremendo? 🫠", "Cuidado... tô logo ali em cima 👑",
  "Cada ponto meu é um pesadelo seu 😱", "Buuu! Acertei mais que você 👻",
];
...
function GhostLeader({ leader }) {
  ...
  useEffect(() => {
    ...
    const p = setInterval(() => { setFadePhrase(false); setTimeout(() => { setPhraseIdx(i => (i + 1) % SCARY_PHRASES.length); setFadePhrase(true); }, 400); }, 5000);
  ...
  return (
    <div style={{ position:'fixed',bottom:16,right:12,zIndex:999,display:'flex',flexDirection:'column',alignItems:'center',opacity:0.5,transition:'transform 0.8s ease-in-out',transform:bounce?'translateY(-8px)':'translateY(4px)',pointerEvents:'none' }}>
      <div style={{ position:'relative',width:90,height:110 }}>
        ...
        <svg viewBox="0 0 100 120" style={{ width:90,height:110,filter:'drop-shadow(0 4px 12px rgba(0,0,0,0.15))' }}>
          <defs>
            <radialGradient id="ghostGlow" cx="50%" cy="30%" r="70%"><stop offset="0%" stopColor="#e0e7ff"/><stop offset="50%" stopColor="#c7d2fe"/><stop offset="100%" stopColor="#a5b4fc"/></radialGradient>
            {leader.avatar_url_1 && <pattern id="leaderPhoto" patternUnits="objectBoundingBox" width="1" height="1"><image href={leader.avatar_url_1} x="0" y="0" width="44" height="44" preserveAspectRatio="xMidYMid slice"/></pattern>}
          </defs>
          <path d="M50 5 C20 5 5 25 5 50 L5 95 L15 85 L25 95 L35 85 L50 95 L65 85 L75 95 L85 85 L95 95 L95 50 C95 25 80 5 50 5Z" fill="url(#ghostGlow)" stroke="#818cf8" strokeWidth="1.5" opacity="0.9"/>
          {leader.avatar_url_1 ? <circle cx="50" cy="42" r="22" fill="url(#leaderPhoto)" opacity="0.9"/> : <><circle cx="50" cy="42" r="22" fill={color} opacity="0.9"/><text x="50" y="48" textAnchor="middle" fill="white" fontSize="18" fontWeight="800" fontFamily="sans-serif">{initials}</text></>}
          <circle cx="25" cy="70" r="3" fill="#4f46e5" opacity="0.4"/><circle cx="75" cy="70" r="3" fill="#4f46e5" opacity="0.4"/>
          <path d="M35 78 Q50 85 65 78" fill="none" stroke="#4f46e5" strokeWidth="1.5" opacity="0.3"/>
        </svg>
      </div>
      <div style={{ textAlign:'center',marginTop:-2,fontFamily:"'DM Sans', sans-serif" }}><div style={{ fontSize:12,fontWeight:800,color:'#0F172A' }}>{leader.name} · {leader.total}pts</div></div>
      <div style={{ marginTop:4,background:'rgba(79,70,229,0.1)',border:'1px solid rgba(79,70,229,0.2)',borderRadius:8,padding:'3px 10px',maxWidth:160,textAlign:'center',opacity:fadePhrase?1:0,transition:'opacity 0.4s' }}>
        <span style={{ fontFamily:"'DM Sans', sans-serif",fontSize:10,fontStyle:'italic',color:'#4338ca',fontWeight:600 }}>{SCARY_PHRASES[phraseIdx]}</span>
      </div>
    </div>
  );
}
...
function DeadlineBanner({ phase, matches }) {
  const openCount = (matches || []).filter(m => !matchHasStarted(m.match_date, m.match_time)).length;
  const totalCount = (matches || []).length;
  const allStarted = openCount === 0 && totalCount > 0;

  return (
    <div className={`rounded-xl p-3 px-4 mb-4 flex justify-between items-center flex-wrap gap-2 ${allStarted ? 'bg-red-50 border border-red-200' : 'bg-primary-50 border border-primary-200'}`}>
      <div>
        <div className="text-xs text-dark-500">
          {allStarted ? 'Todos os jogos desta rodada já começaram' : `${openCount} de ${totalCount} jogos ainda abertos para palpite`}
        </div>
      </div>
      <div className={`px-3 py-1 rounded-full text-xs font-bold ${allStarted ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
        {allStarted ? '🔒 Encerrado' : `⚽ ${openCount} abertos`}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-dark-50 rounded-lg p-3 border border-dark-200">
      <div className="flex justify-between items-center mb-2">
        ...
        {used.length > 0 && <button onClick={() => { const empty = {1:'',2:'',3:'',4:''}; setPositions(empty); onSave(group, empty); }} className="text-[10px] text-red-500 hover:text-red-700">🔄 Limpar</button>}
      </div>
      {[1,2,3,4].map(pos => (
...
function MatchRow({ match, guess, onGuessChange, locked }) {
  return (
    <div className={`match-card ${match.is_brasil ? 'match-card-brasil' : 'match-card-normal'} ${locked ? 'opacity-50' : ''}`}>
      <div className="flex justify-between items-center mb-1">
        <span className="text-[10px] text-dark-500">
          {match.group_letter ? `Grupo ${match.group_letter} · ` : ''}{match.match_time}
          {locked && <span className="ml-1 text-red-500 font-bold">🔒</span>}
        </span>
...
function KnockoutRow({ match, guess, onGuessChange, locked }) {
  return (
    <div className={`match-card ${match.is_brasil ? 'match-card-brasil' : 'match-card-normal'} ${locked ? 'opacity-50' : ''}`}>
      <div className="flex justify-between items-center mb-1">
        <span className="text-[10px] text-dark-500">{match.match_time}{locked && <span className="ml-1 text-red-500 font-bold">🔒</span>}</span>
...
export default function PalpitesPage({ user }) {
  ...
  // Initial/classif use phase deadline, matches use per-game time
  const initialLocked = isDeadlinePassed('initial');
  const classifLocked = isDeadlinePassed('group_class');
  ...
  async function loadData() {
    ...
    if (section === 'inicial') {
      const { data } = await supabase.from('initial_predictions').select('*').eq('user_id', user.id).maybeSingle();
  ...
  async function saveInitial() {
    ...
    if (initialLocked) { setMessage('Prazo encerrado!'); return; }
    setSaving(true);
    const { error } = await supabase.from('initial_predictions').upsert({ user_id: user.id, champion: champ, vice: vice, third_place: third }, { onConflict: 'user_id' });
    setSaving(false);
    setMessage(error ? 'Erro: ' + error.message : '✅ Palpite inicial salvo!');
  }

  async function saveGroupClass() {
    if (classifLocked) { setMessage('Prazo encerrado!'); return; }
    setSaving(true);
    let saved = 0;
    for (const [grp, pos] of Object.entries(groupClass)) {
      if (pos[1] && pos[2] && pos[3] && pos[4]) {
        await supabase.from('group_class_guesses').upsert({ user_id: user.id, group_letter: grp, pos_1: pos[1], pos_2: pos[2], pos_3: pos[3], pos_4: pos[4] }, { onConflict: 'user_id,group_letter' });
        saved++;
      }
    }
    setSaving(false);
  ...
  async function saveMatchGuesses() {
    ...
    for (const match of matches) {
      // Skip locked matches (already started)
      if (matchHasStarted(match.match_date, match.match_time)) { skipped++; continue; }
    ...
    setSaving(false);
    if (saved > 0 && skipped > 0) setMessage(`✅ ${saved} palpite(s) salvo(s)! ${skipped} jogo(s) já começaram (🔒)`);
    else if (saved > 0) setMessage(`✅ ${saved} palpite(s) salvo(s)!`);
    else if (skipped > 0) setMessage('⚠️ Todos os jogos desta rodada já começaram');
  ...
  async function clearAll() {
    ...
    if (section === 'inicial') { setChamp(''); setVice(''); setThird(''); await supabase.from('initial_predictions').delete().eq('user_id', user.id); setMessage('Palpite inicial limpo!'); }
      ...
      // Only clear open matches
      const openMatchIds = matches.filter(m => !matchHasStarted(m.match_date, m.match_time)).map(m => m.id);
      if (openMatchIds.length > 0) await supabase.from('match_guesses').delete().eq('user_id', user.id).in('match_id', openMatchIds);
      const newGuesses = { ...guesses };
      openMatchIds.forEach(id => delete newGuesses[id]);
      setGuesses(newGuesses);
      setMessage('Palpites dos jogos abertos limpos!');
    }
  }

  const isKnockout = ['32avos', 'oitavas', 'quartas', 'semi', 'terceiro', 'final'].includes(section);
  const hasOpenMatches = matches.some(m => !matchHasStarted(m.match_date, m.match_time));
  ...
  return (
    <div className="max-w-[900px] mx-auto p-5">
      ...
      {['r1','r2','r3','32avos','oitavas','quartas','semi','terceiro','final'].includes(section) && (
        <DeadlineBanner phase={current.phase} matches={matches} />
      )}

      {section === 'inicial' && (
        <div className={`rounded-xl p-3 px-4 mb-4 flex justify-between items-center ${initialLocked ? 'bg-red-50 border border-red-200' : 'bg-primary-50 border border-primary-200'}`}>
          <span className="text-xs text-dark-500">Prazo para palpite inicial</span>
          <span className={`px-3 py-1 rounded-full text-xs font-bold ${initialLocked ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
            {initialLocked ? '🔒 Encerrado' : '⏰ Aberto'}
          </span>
        </div>
      )}

      {message && <div className="mb-3 p-2 rounded-lg bg-primary-50 border border-primary-200 text-sm text-primary-800 animate-fade-in">{message}</div>}
      ...
      {section === 'inicial' && (
        <div className="card animate-slide-up">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <FlagSelect label="🏆 Campeão (60 pts)" value={champ} onChange={v => !initialLocked && setChamp(v)} />
            <FlagSelect label="🥈 Vice (40 pts)" value={vice} onChange={v => !initialLocked && setVice(v)} />
            <FlagSelect label="🥉 3º Lugar (20 pts)" value={third} onChange={v => !initialLocked && setThird(v)} />
          </div>
          {!initialLocked && (
      ...
      {['r1', 'r2', 'r3'].includes(section) && (
        <div className="card animate-slide-up">
          ...
          <div className="grid gap-2">
            {matches.map(m => {
              const locked = matchHasStarted(m.match_date, m.match_time);
              return <MatchRow key={m.id} match={m} guess={guesses[m.id]} onGuessChange={handleGuessChange} locked={locked} />;
            })}
          </div>
          {hasOpenMatches && matches.length > 0 && (
            <div className="flex gap-2 mt-4">
      ...
      {isKnockout && (
        <div className="card animate-slide-up">
          ...
          <div className="grid gap-3">
            {matches.map(m => {
              const locked = matchHasStarted(m.match_date, m.match_time);
              return <KnockoutRow key={m.id} match={m} guess={guesses[m.id]} onGuessChange={handleGuessChange} locked={locked} />;
            })}
          </div>
          {hasOpenMatches && matches.length > 0 && (
            <div className="flex gap-2 mt-4">
