...
export default function ResenhaPage({ user }) {
  ...
  async function loadData() {
    // Load users first
    const { data: users } = await supabase.from('users').select('id, name, avatar_url_1');
    const uMap = {};
  ...
  async function sendMessage() {
    ...
    const { error } = await supabase.from('chat_messages').insert({
      user_id: user.id, message_type: 'user', content: newMsg.trim(),
    });
    if (!error) setNewMsg('');
  }
    ...
    <div className="max-w-[900px] mx-auto p-5">
      <div className="card p-0 overflow-hidden flex flex-col" style={{ height: 'calc(100vh - 160px)', minHeight: 400 }}>
        ...
        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-1.5">
          {loading && <div className="text-center py-10 text-dark-500 text-sm">Carregando mensagens...</div>}

          {messages.map((msg, i) => {
            const isSummary = msg.message_type === 'summary';
            const isSystem = msg.message_type === 'system' || msg.message_type === 'zoeira';
            const isOwn = msg.user_id === user.id;
            const msgUser = usersMap[msg.user_id];
            const name = msgUser?.name || 'Sistema';
            ...
            if (isSummary) {
              return (
                <div key={msg.id || i} className="self-center w-full animate-fade-in" style={{ maxWidth: '95%' }}>
                  <div style={{
                    background: 'linear-gradient(135deg, #FFF7ED, #FEF3C7)',
                    border: '2px solid #F59E0B',
                    borderRadius: 12, padding: 16,
                  }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: '#92400E', marginBottom: 8, textAlign: 'center' }}>🍯 RESUMO DA RODADA 🍯</div>
                    <div style={{
            ...
            return (
              <div key={msg.id || i} className={`flex gap-2 max-w-[80%] animate-fade-in ${isOwn ? 'self-end flex-row-reverse' : 'self-start'}`}>
                <Avatar name={name} size={30} url={msgUser?.avatar_url_1} />
                <div className={`rounded-xl px-3 py-2 ${isOwn ? 'bg-primary-600 text-white rounded-tr-none' : 'bg-dark-100 text-dark-900 rounded-tl-none'}`}>
                  <div className={`text-[11px] font-bold mb-0.5 ${isOwn ? 'text-primary-200' : 'text-primary-600'}`}>{name}</div>
                  <div className="text-sm leading-relaxed">{msg.content}</div>
                  <div className={`text-[9px] mt-1 text-right ${isOwn ? 'text-primary-300' : 'text-dark-400'}`}>{time}</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="p-3 border-t border-dark-200 bg-dark-50 flex gap-2 items-center">
          <input value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={handleKeyDown}
            placeholder="Manda a resenha..." className="input-field flex-1 py-2.5 bg-white" />
          <button onClick={sendMessage} className="bg-primary-600 text-white px-4 py-2.5 rounded-lg font-bold text-sm hover:bg-primary-700 transition-all whitespace-nowrap">Enviar</button>
        </div>
      </div>
    </div>
  );
